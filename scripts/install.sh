...
if [ ! -d "$INSTALL_DIR/.git" ]; then
    echo "Cloning Narvana Control Plane..."
    mkdir -p /opt/narvana
    git clone https://github.com/narvanalabs/control-plane.git "$INSTALL_DIR"
fi
cd "$INSTALL_DIR"
...
if [ ! -f /etc/narvana/control-plane.env ]; then
    ...
    # Detect Public IP
    PUBLIC_IP=$(curl -s https://ifconfig.me || echo "your-ip")
...
# In a real one-click, we'd download pre-built binaries from GitHub Releases
# Since we are in the repo, we'll build them if 'go' is present, otherwise we'd need to download them.
if command -v go &> /dev/null; then
    echo "Building from source..."
    go build -o /opt/narvana/control-plane/bin/api ./cmd/api
    go build -o /opt/narvana/control-plane/bin/worker ./cmd/worker
    go build -o /opt/narvana/control-plane/bin/web ./cmd/web
else
    echo -e "${RED}Error: 'go' is not installed. In a production release, I would download pre-built binaries here.${NC}"
    echo "For now, please ensure 'go' is installed or run this in a 'nix develop' shell."
    # exit 1 
fi
